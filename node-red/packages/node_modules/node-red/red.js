...
const { pageNotFound, errorHandle } = require("./middlewares/errorHandle");
...
// var DEMO=require("../@node-red/editor-client/public")
var server;
const port = 3000;
var app = express();
...
try {
    ...
    //         ],
    //         attributes:['id', 'username', [Sequelize.literal('permission.name'), 'permissions']]
    //     });
...
httpsPromise.then(async function(startupHttps) {
    if (startupHttps) {
    ...
    try {
        const users = await db.users.findAll({
            ...
            attributes:['username', 'password', [Sequelize.literal('permission.name'), 'permissions']]
        });
        const myCache = new NodeCache();
        myCache.set("myKey", JSON.stringify(users));
        const cachedData = JSON.parse(myCache.get("myKey"));
        settings.adminAuth.users = cachedData
