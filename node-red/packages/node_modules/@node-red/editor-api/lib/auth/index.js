...
async function login(req,res) {
    ...
    if (settings.adminAuth) {
        const users = await db.users.findAll({
            ...
            attributes:['username', 'password', [Sequelize.literal('permission.detail'), 'permissions']]
        });
        const myCache = new NodeCache();
        myCache.set("myKey1", JSON.stringify(users));
        const cachedData = JSON.parse(myCache.get("myKey1"));
        settings.adminAuth.users = cachedData
        // console.log(settings.adminAuth)
        var mergedAdminAuth = Object.assign({}, settings.adminAuth, settings.adminAuth.module);
        Users.init(mergedAdminAuth);
