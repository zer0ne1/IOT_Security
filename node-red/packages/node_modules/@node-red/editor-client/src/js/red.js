    ...
    function loadFlows(done) {
        ...
        $.ajax({
            ...
            success: function(nodes) {
                if (nodes) {
                    // const tabsWithSpecifiedInfo = nodes.result.flows.filter(item => item.type === 'tab' && item.info === nodes.user );
                    // const tabIds = tabsWithSpecifiedInfo.map(item => item.id);
                    // const nonTabItemsLinkedToTabs = nodes.result.flows.filter(item => item.type !== 'tab' && tabIds.includes(item.z));
                    // const filteredData = [...tabsWithSpecifiedInfo,...nonTabItemsLinkedToTabs];
                    var currentHash = window.location.hash;
                    RED.nodes.version(nodes.result.rev);
                    loader.reportProgress(RED._("event.importFlows"),90 )
                    try {
                        RED.nodes.import(nodes.result.flows);
                        // RED.nodes.import(nodes);
                        // RED.nodes.import(filteredData,123);
