...
function loadFlows(opts) {
    var config;
    console.log("kiểm tra userrrrrrrrrrrrrrrrr", opts);
    return storage.getFlows(opts).then(function(_config) {
        config = _config;
        log.debug("loaded flow revision: "+config.rev);
        return config
        // return credentials.load(config.credentials).then(function() {
        //     events.emit("runtime-event",{id:"runtime-state",retain:true});
        //     console.log("Kiểm tra xem là gì nào",config )
        //     return config;
        // });
    }).catch(function(err) {
...
function getFlows(opts) {
    console.log("Kiểm tra optsssssssss zahi", opts)
    const activeConfig =loadFlows(opts?.user?.username).then(function(_config) {
        config = clone(_config.flows)
...
function getFlow(id) {
    var flow;
    // console.log(">>>>>>>>>>>>>>>> CHECK VAR",activeFlowConfig)
    if (id === 'global') {
