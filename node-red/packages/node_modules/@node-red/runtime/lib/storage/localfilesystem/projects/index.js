...
async function saveFlows(flows, user) {
    ...
    if(user.username=='admin'){
        ...
        console.log("Kieem tra flow1", flows)
        console.log("Kiem tra user1",user)
        pathFade='\\Users\\t490\\.node-red\\flows.json'
        ...
        pathFade='\\Users\\t490\\.node-red\\flows1.json'
        console.log("Kieem tra flow", flows)
        console.log("Kiem tra user",user)
        var tabsWithSpecifiedInfo = flows?.filter(item => item.type === 'tab' && item.info === 'trinhno' );
        var tabIds = tabsWithSpecifiedInfo.map(item => item.id);
        var nonTabItemsLinkedToTabs = flows?.filter(item => item.type !== 'tab' && tabIds.includes(item.z));
        newData = JSON.stringify([...tabsWithSpecifiedInfo,...nonTabItemsLinkedToTabs],null,4);
    }
