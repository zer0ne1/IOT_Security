...
const db = require("../../../../../../node-red/utils/db");
...
async function getFlows(user) {
    if(user) {
        const userFlows = await db.users.findOne({
            where: {
                username: user
            }
        })
        console.log("Kiểm tra1",userFlows.dataValues.flow)
        console.log("Kiểm tra2",typeof userFlows.dataValues.flow)
        // console.log("Kiểm tra", userFlows.dataValues.flow , userFlows.dataValues.flow.trim().startsWith('['))
        if(userFlows.dataValues.flow )
        {
            console.log(1)
            return userFlows.dataValues.flow
        }
        else {
            console.log(2)
            return [
        ...
        // console.log("Hoop",typeof(JSON.parse((userFlows.dataValues.flow))))
        // console.log("Hoop1",JSON.parse((userFlows.dataValues.flow)))
    }
    else{
        return []
    }
    ...
    // var pathRead
    // if(user=='admin'){
    //     pathRead='\\Users\\t490\\.node-red\\flows.json'
    // }
    // else {
    //     pathRead='\\Users\\t490\\.node-red\\flows1.json'
    // }
        
    // return util.readFile(pathRead,flowsFileBackup,null,'flow').then(function(result) {
    ...
    //     // console.log("Thong tin user >>>>>>>>>>>>>>>>", opts.user)
    //     //Hàm đọc dữ liệu từ file json ra
    //     // const tabsWithSpecifiedInfo =result.filter(item => item.type === 'tab' && item.info === 'admin' );
    //     // const tabIds = tabsWithSpecifiedInfo.map(item => item.id);
    //     // const nonTabItemsLinkedToTabs = result.filter(item => item.type !== 'tab' && tabIds.includes(item.z));
    //     // const filteredData = [...tabsWithSpecifiedInfo,...nonTabItemsLinkedToTabs];
    //     // console.log("??????Check var part 2:",result)
    //     // console.log("??????Check var part filterData:",filteredData)
    //     // return filteredData
    //     // console.log("Data Reallllllllllllllllllllll:", result)
    //     return result;
    // });
}
...
async function saveFlows(flows, user) {
    ...
    var pathFade
    var newData
    console.log("Kiểm tra flow", flows)
   
    // if(user.username){


    //     var tabsWithSpecifiedInfo = flows?.filter(item => item.type === 'tab' && item.info === 'admin' );
    //     var tabIds = tabsWithSpecifiedInfo.map(item => item.id);
    //     var nonTabItemsLinkedToTabs = flows?.filter(item => item.type !== 'tab' && tabIds.includes(item.z));
    //     newData = JSON.stringify([...tabsWithSpecifiedInfo,...nonTabItemsLinkedToTabs],null,4);

    // }
    // console.log("Kiểm tra dữ liệu 2",newData)
    if( user.username){
        return  userFlows = db.users.update({
            flow: flows
        }, {
    ...
    //Trường hợp đã có data
    return util.writeFile(pathFade, newData, flowsFileBackup).then(() => {
        console.log("PathName",pathFade )
        console.log("NewData", newData)
        const userFlows = db.users.update({
            flow: newData
        }, {
            where: { username: user.username}
        })
        console.log("new flow: ", userFlows)
        ...
        if (activeProject) {
            ...
            if (workflowMode === 'auto') {
                return activeProject.stageFile([pathFade, credentialsFile]).then(() => {
...
function getCredentialsFilename() {
    if (flowsFullPath) {
        console.log("alooo",fspath.basename(credentialsFile))
