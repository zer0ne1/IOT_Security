...
async function getFlows(user) {
    if(user) {
        ...
        if(userFlows.dataValues.flows) {
            // console.log("flows: ", JSON.parse(Buffer.from(userFlows.dataValues.flows, 'base64').toString('ascii')))
            return JSON.parse(Buffer.from(userFlows.dataValues.flows, 'base64').toString('ascii'))
        }
        else {
            return [
                {
        ...
        //Thay vì đọc trực tiếp từ CSDL, ta sẽ coppy vào file để đọc như bình thường
...
async function saveFlows(flows, user) {
    ...
    if(flows){
        util.writeFile(flowsFullPath, flowData, flowsFileBackup).then(() => {
        ...
        return userFlows = db.users.update({
            flows: Buffer.from(JSON.stringify(flows)).toString('base64')
        }, {
